...
func runStep(parent context.Context, cfg Config, requestedClients int, statsClient *StatsClient, rng *rand.Rand) (StepResult, error) {
	...
	if err := waitForServerStabilization(stepCtx, cfg, statsClient); err != nil {
		return StepResult{TargetClients: targetClients, TargetRooms: targetRooms, StartedAtRFC3339: started.UTC().Format(time.RFC3339), EndedAtRFC3339: time.Now().UTC().Format(time.RFC3339), DurationSeconds: int64(time.Since(started).Seconds()), FailReason: fmt.Sprintf("server stabilization failed: %v", err)}, err
	}
...
func waitForServerStabilization(ctx context.Context, cfg Config, client *StatsClient) error {
	...
	deadline := time.Now().Add(time.Duration(cfg.PreRampStabilizeSeconds) * time.Second)
	...
	for {
		...
		if time.Now().After(deadline) {
			...
			return fmt.Errorf("active clients are not idle yet")
		}
