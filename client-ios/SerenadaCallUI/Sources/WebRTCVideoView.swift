...
struct WebRTCVideoView: UIViewRepresentable {
    ...
    func makeUIView(context: Context) -> UIView {
        ...
        case .local:
            ...
            Task { @MainActor in
                rendererProvider.attachLocalRenderer(renderer)
            }
        ...
        case .remote:
            ...
            Task { @MainActor in
                rendererProvider.attachRemoteRenderer(renderer)
            }
            context.coordinator.renderer = renderer
            return renderer
        case .remoteForCid(let cid):
            ...
            renderer.delegate = context.coordinator
            Task { @MainActor in
                rendererProvider.attachRemoteRenderer(renderer, forCid: cid)
            }
            context.coordinator.renderer = renderer
            return renderer
        }
    ...
    public static func dismantleUIView(_ uiView: UIView, coordinator: Coordinator) {
        ...
        Task { @MainActor in
            ...
            case .local:
                coordinator.rendererProvider?.detachLocalRenderer(renderer)
            case .remote:
    ...
    public final class Coordinator: NSObject {
        let kind: Kind
        weak var rendererProvider: (any CallRendererProvider)?
        var isMirrored: Bool = false
        weak var renderer: AnyObject?
        let onVideoSizeChanged: ((CGSize) -> Void)?

        init(kind: Kind, rendererProvider: CallRendererProvider, onVideoSizeChanged: ((CGSize) -> Void)?) {
            self.kind = kind
            self.rendererProvider = rendererProvider
            self.onVideoSizeChanged = onVideoSizeChanged
            super.init()
