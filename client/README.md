...
- Screen share can run as an independent **content** stream so camera and screen share can be active at once. The web SDK surface includes `cameraEnabled` and `content` on participant state, plus `session.getRemoteContentStream(cid)` / `getRemoteCameraStream(cid)` / `getLocalContentStream()` (the existing `getRemoteStream(cid)` stays camera-specific). It is gated behind `SerenadaConfig.enableIndependentContentVideo` (SDK default `false`; the bundled web app opts in); with the flag off, screen share reuses the camera path exactly as today. See the root README's "Independent Content Video" section and the protocol spec for details.
