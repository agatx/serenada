...
- Screen share is becoming an independent **content** stream so camera and screen share can run at once. The web SDK surface is in place — `cameraEnabled` and `content` on participant state, plus `session.getRemoteContentStream(cid)` / `getRemoteCameraStream(cid)` / `getLocalContentStream()` (the existing `getRemoteStream(cid)` stays camera-specific). It is gated behind `SerenadaConfig.enableIndependentContentVideo` (default `false`, **not yet enabled**); with the flag off, screen share reuses the camera path exactly as today. See the root README's "Independent Content Video" section and the protocol spec for details.
