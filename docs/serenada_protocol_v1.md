...
```json
{
  ...
  "to": "C-c3d4...",
  "payload": {
    "offerId": "C-a1b2...:C-c3d4...:1735171200000:1",
    "sdp": "v=0\r\n..."
  }
}
```
...
**Notes**
- `payload.offerId` identifies the offer/answer exchange that produced the candidate. It is generated by the offerer and echoed by the answerer.
...
- Generate an `offerId` for every local offer, include it in the `offer`, echo it in the matching `answer`, and attach it to local ICE for that negotiation.
- Apply perfect negotiation glare handling: the deterministic offerer is impolite and ignores colliding remote offers; the other side is polite, rolls back any already-pending local offer if needed, applies the remote offer, and answers. Non-offerers must not create fallback offers.
- Drop answers that arrive outside `have-local-offer` or whose `offerId` does not match the pending local offer. Buffer ICE for a future `offerId` until the corresponding offer or answer is accepted, then flush it.
