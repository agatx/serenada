...
- In-call diagnostics panel in the top-left corner, toggled with a double-tap in that corner zone (same gesture as web), showing the same working-copy web diagnostics sections/metrics (`Connection`, `Latency`, `Audio Quality`, `Video Quality`) including active WebRTC transport, RTT/path, inbound/outbound audio and video codec names, packet loss, jitter/playout delay, bitrate, FPS, freezes, and retransmit
