...
- In-call diagnostics panel in the top-left corner, toggled with a double-tap in that corner zone (same gesture as web), showing the same working-copy web diagnostics sections/metrics (`Connection`, `Latency`, `Audio Quality`, `Video Quality`) including active WebRTC transport, RTT/path, inbound/outbound audio and video codec MIME types, packet loss, jitter/playout delay, bitrate, FPS, freezes, and retransmit
...
- Independent content video (screen share) SDK surface in `serenada-core`: `RemoteParticipant.cameraEnabled`/`content` and `CallState.localCameraEnabled`/`localContent` (camera-specific state alongside legacy `videoEnabled`), the signaled `mediaPolicy.videoMediaEnabled` session policy, and content renderer APIs `SerenadaSession.attachRemoteContentRenderer(renderer, cid)`/`detachRemoteContentRenderer(...)` plus `attachLocalContentRenderer(renderer)`/`detachLocalContentRenderer(...)` (existing `attachRemoteRenderer` stays camera-specific). Gated behind `SerenadaConfig.enableIndependentContentVideo` (default `false` for SDK integrators, enabled by the bundled app); with the flag off, screen share behaves exactly as today. See the root README and protocol spec for the cross-platform contract.
...
```bash
cd client-android
ls serenada-core/libs/libwebrtc-7827-universal.aar
```
...
```
The script outputs:
`/opt/webrtc-build/artifacts/libwebrtc-7827-universal-curlroots.aar`
...
After replacing `serenada-core/libs/libwebrtc-7827-universal.aar`, update the pinned SHA-256 file used by Gradle verification:
```bash
cd client-android
shasum -a 256 serenada-core/libs/libwebrtc-7827-universal.aar | awk '{print $1}' > serenada-core/libs/libwebrtc-7827-universal.aar.sha256
```
...
```
This publishes:
- `app.serenada:libwebrtc-7827-universal:0.9.3`
- `app.serenada:core:0.9.3`
- `app.serenada:call-ui:0.9.3`
